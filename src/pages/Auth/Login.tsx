...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (from === '/' && data.user) {
        ...
        if (role === 'admin' || role === 'moderator') redirectPath = '/admin/dashboard';
        else redirectPath = '/profile';
      }

      toast('Welcome back to Xorvin!', 'success');
  ...
  return (
    <>
      <Helmet>
        <title>Sign In — Xorvin</title>
      </Helmet>
      
      <div className="glass-card rounded-3xl p-8 sm:p-10 w-full border border-white/10 shadow-glow-sm">
        <div className="text-center mb-8">
          <h1 className="heading-sm text-white mb-2">Welcome Back</h1>
          <p className="text-white/60 text-sm font-inter">Sign in to continue to Xorvin</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mb-6">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5 mx-1">
              ...
              <Link to="/auth/forgot-password" className="text-xs text-xorvin-accent hover:underline">Forgot?</Link>
            </div>
            <div className="relative">
        ...
        <div className="relative mb-6 text-center">
          ...
          <span className="relative px-4 text-xs bg-[#0b1626] text-white/40 uppercase tracking-wider">Or continue with</span>
        </div>
        ...
        <p className="text-center text-sm text-white/60">
          Don't have an account? <Link to="/auth/register" className="text-xorvin-accent hover:underline font-medium">Sign up</Link>
        </p>
      </div>
    </>
  );
